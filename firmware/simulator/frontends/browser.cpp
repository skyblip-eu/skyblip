...
KEEPALIVE void simulator_backlight(int on) { g_simulator.product().screen().set_backlight(on != 0); }
...
KEEPALIVE void simulator_add_aircraft(int north_m, int east_m, int up_m, int speed_mps, int track_deg) {
...
KEEPALIVE int simulator_capabilities() { return static_cast<int>(g_simulator.product().capabilities()); }
