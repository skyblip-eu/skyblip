// devices/soc/zephyr/ble.cpp — Zephyr BT GATT implementation of the companion
// link (§6). The ONLY place GATT lives. A vendor service
// with two characteristics: NMEA (notify → EFB) and CONFIG (write in / notify
// out). Inbound CONFIG writes are queued for the shell to hand to App.
...
Status BleLink::send(Endpoint ep, ConstByteSpan bytes) {
...
bool BleLink::pop_rx(RxFrame& out) {
    k_spinlock_key_t key = k_spin_lock(&g_lock);
