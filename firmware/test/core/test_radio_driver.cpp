// SX1262 driver recovery tests against the behavioral fake with fault injection
// (3-ARCHITECTURE §8: the exact class of intermittent bug that is hell on HW).
#include "devices/drivers/sx1262.h"
#include "devices/host/fake_sx1262.h"
#include "doctest/doctest.h"

using namespace skyblip;
using namespace skyblip::drivers;

static Sx1262 make(host::FakeSx1262& f) {
    return Sx1262(f, f, f.busy_pin, f.reset_pin, f.dio1_pin);
}

TEST_CASE("radio: begin configures the TCXO (DIO3) and RF switch (DIO2) — T-Echo wiring") {
    host::FakeSx1262 fake;
    Sx1262 r = make(fake);
    CHECK(r.begin() == Status::Ok);
    // Without these two the SX1262 has no clock / no antenna path on this board.
    CHECK(fake.saw_cmd(sx::kSetDio2AsRfSwitch));
    CHECK(fake.saw_cmd(sx::kSetDio3AsTcxoCtrl));
    CHECK(fake.saw_cmd(sx::kCalibrate));
}
...
TEST_CASE("radio: a CRC-error RX is reported as CrcError, NOT delivered (§8.5)") {
    host::FakeSx1262 fake;
    Sx1262 r = make(fake);
    r.begin();
    r.configure_mband(MbandConfig{});
    ...
    fake.queue_rx(pkt, 4, /*crc_error=*/true);
    uint8_t buf[32];
    RadioEvent ev = r.poll(buf, sizeof(buf));
